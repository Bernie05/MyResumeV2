...
type StudioSection = (typeof STUDIO_SECTIONS)[number]["value"];
...
function textToCsv(value: string): string[] {
  return value
    .split(",")
    .map((item) => item.trim())
    .filter(Boolean);
...
function csvToText(value: string[]): string {
  return value.join(", ");
}
...
function replaceItemAtIndex<T>(items: T[], index: number, nextItem: T): T[] {
...
function createEmptyExperienceItem(items: ExperienceItem[]): ExperienceItem {
  return {
    id: getNextId(items),
    company: "",
    position: "",
...
function createEmptyEducationItem(items: EducationItem[]): EducationItem {
  return {
    id: getNextId(items),
    school: "",
    degree: "",
...
function createEmptyProjectItem(items: ProjectItem[]): ProjectItem {
  return {
    id: getNextId(items),
    name: "",
    description: "",
...
function createEmptyPortfolioItem(items: PortfolioItem[]): PortfolioItem {
  return {
    id: getNextId(items),
    title: "",
    description: "",
...
function createEmptySkillCategory(): SkillCategory {
  return {
    category: "",
...
export default function ResumeStudio({ initialResume }: ResumeStudioProps) {
  const router = useRouter();
  const { isDarkMode } = useThemeContext();
  const [activeSection, setActiveSection] =
    useState<StudioSection>("personalInfo");
  ...
  useEffect(() => {
    ...
    try {
      const parsed = JSON.parse(savedValue) as ResumeData;
      setDraft(parsed);
      ...
      setSavedDraft(baseline);
      setError(
        "Saved studio draft was invalid and has been reset to the baseline resume.",
      );
    } finally {
      setIsHydrated(true);
    }
  ...
  if (!isHydrated) {
    return (
      ...
      >
        <Typography sx={{ color: isDarkMode ? "#cbd5e1" : "#334155" }}>
          Loading studio draft...
        </Typography>
      </Box>
    );
  }
    ...
    >
      ...
      >
        <Stack spacing={2} sx={{ px: { xs: 2, md: 3 }, py: 2 }}>
          ...
          >
            <Stack spacing={0.75}>
              ...
              <Typography sx={{ color: isDarkMode ? "#94a3b8" : "#475569" }}>
                Edit the local draft, preview the real resume UI, and keep the
          ...
          <Tabs
            value={activeSection}
            onChange={(_, value: StudioSection) => setActiveSection(value)}
            variant="scrollable"
            scrollButtons="auto"
          >
