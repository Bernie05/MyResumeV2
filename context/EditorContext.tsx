 ...
 * @param value - The value to be provided to the context. This should be an object that adheres to the EditorContextValue interface.
...
/**
 * useEditor is a custom React hook that allows components to access the EditorContext. It uses the useContext hook to retrieve the current value of the EditorContext and returns it. If the context is not available (i.e., if the component using this hook is not wrapped in an EditorProvider), it will return null. This hook provides a convenient way for components to access the editor-related state and functions defined in the context.
 * @returns The current value of the EditorContext, which should be an object adhering to the EditorContextValue interface, or null if the context is not available.
 */
